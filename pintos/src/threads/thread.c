...
int enter_once;                 /*ensure initializing nice and recent cpu is run only one time*/
...
/*executed only when scheduler is bsd
calculate the recent_value and the load_avg value*/ 
if(thread_mlfqs)
{
   if(t->status == THREAD_RUNNING)     //increase value of recent cpu by one if only the current thread is running
    t->recent_cpu =REAL_ADD_INT (t->recent_cpu, 1);

  if(timer_ticks() % TIME_FREQ == 0)   //every one second recalculate the load_average and the recent_cpu for all threads
  { 
...
if (timer_ticks() % 4 == 0) //every four ticks  update priority
...
/*this function is used with bsd scheduler to update the priority 
 for all elements in the system include blocked and running threads*/
               ...
               thread_func *function, void *aux) 
{
  ...
  sf->ebp = 0;
  /*executed only when scheduler is bsd
    the first thread nice and recent cpu is equal zero
    other threads's nice value and recent cpu is inherited from parent thread */ 
  if(thread_mlfqs)
  {
	  if (!enter_once)  
	  {
...
thread_unblock (struct thread *t) 
{
  ...
  if(thread_current() != idle_thread && thread_current()->priority < t->priority){
     if(intr_context())         //in case of there is an interrupt thread_yield occur after interrupt end
	 intr_yield_on_return (); 
	else thread_yield(); 	
   }
  intr_set_level (old_level);
...
/*if the thread is priority is less than MIN_PRI return MIN_PRI
  else if thread is priority is more than MAX_PRI return MAX_PRI
  else return the same value
  to check that the priority is lie in the valid bound */
...
next_thread_to_run (void) 
{
  ...
  {
    if(thread_mlfqs)
    /*if bsd is used then the priority is change every four ticks for all elements in the list 
      so sorting then will take much time so when calling scheduler get the max element 
      lie in the ready list .*/
    return list_entry (list_max(&ready_list, less_prio_for_prio, NULL),struct thread, elem);      
    /*in case of priority scheduler pop_back is called which runs in O(1)*/
